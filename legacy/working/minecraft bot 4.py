  ...
  !chop, !mine, !collect, !inventory, !task, !stop, etc.
    ...
    def setup_bot_events(self):
        ...
        def handle_spawn():
            ...
            if hasattr(self.bot, 'entity') and self.bot.entity:
                pos = self.bot.entity.position
                print(f"📍 Position: x={pos.x:.1f}, y={pos.y:.1f}, z={pos.z:.1f}")
            self.bot.chat("Hello! Bot is online and ready!")
        
        @self.bot.on("login")
        def handle_login():
            print("🔑 Bot logged in to server")
        
        @self.bot.on("connect")
        def handle_connect():
            print("🌐 Connected to server")
        
        @self.bot.on("spawnReset")
        def handle_spawn_reset():
            print("🔄 Spawn location reset")
        ...
        def handle_message(json_msg, position):
            ...
            if position == "system" or position == "game_info":
                print(f"📢 Server message: {json_msg}")
    ...
    def start_bot(self):
        ...
        print(f"🔐 Auth mode: {self.config['bot']['auth']}")
        print(f"🎮 Version: {self.config['server'].get('version', 'auto')}")
    
        try:
            # Create the bot instance with more options
            ...
            # Wait longer for bot to initialize and add connection timeout
            connection_timeout = 30  # seconds
            start_time = time.time()
        
            while self.running:
                # Check if bot has connected and has entity data
                if hasattr(self.bot, 'entity') and self.bot.entity is not None:
                    print("✅ Bot successfully connected and spawned!")
                    break
                
                # Check for timeout
                if time.time() - start_time > connection_timeout:
                    print("❌ Connection timeout: Bot failed to connect within 30 seconds")
                    print("💡 Possible issues:")
                    print("   - Server is not running")
                    print("   - Wrong server address/port")
                    print("   - Server requires online mode but using offline auth")
            ...
            print("   1. Make sure the Minecraft server is running")
            print("   2. Check the server address and port")
            print("   3. Verify the Minecraft version")
            print("   4. Check if server is in offline mode if using offline auth")
            self.running = False
